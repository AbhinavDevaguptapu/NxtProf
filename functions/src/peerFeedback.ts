...
export const adminGetAllPeerFeedback = onCall(
  ...
  async (request) => {
    ...
    const feedback = await Promise.all(
      givenSnapshot.docs.map(async (doc) => {
        const data = doc.data() as GivenPeerFeedback;
        ...
        if (
          giverDoc.data()?.archived === true ||
          receiverDoc.data()?.archived === true
        ) {
          return null;
        }
...
export const givePeerFeedback = onCall<{
  ...
  const { targetId, projectOrTask, workEfficiency, easeOfWork, remarks } =
    request.data;
  ...
  try {
    ...
    logger.error("Error giving peer feedback", {
      userId: giverId,
      targetId: sanitizedTargetId,
      error: error.message,
      timestamp: new Date().toISOString(),
