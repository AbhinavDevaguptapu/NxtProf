...
export const analyzeTask = onCall<{ prompt: string }>(
    { timeoutSeconds: 120, memory: "512MiB", secrets: ["GEMINI_KEY"], cors: true },
    async (request) => {
        if (!request.auth) {
        ...
        if (sanitizedPrompt.length === 0 || sanitizedPrompt.length > 20000) {
            throw new HttpsError("invalid-argument", "Prompt must be between 1 and 20000 characters.");
        }
...
export const getLearningPointsForEmployee = onCall<{ employeeId: string }>({ cors: true },
    async (request) => {
        if (!request.auth) {
        ...
        if (!isAdmin && request.auth.uid !== sanitizedEmployeeId) {
            throw new HttpsError("permission-denied", "You can only access your own learning points.");
        }
