...
async function getFilteredFeedbackData(
  ...
  if (typeof sheetUrl !== "string")
    throw new HttpsError("not-found", "No feedback sheet URL configured.");
...
export const getFeedbackChartData = onCall<FeedbackRequestData>(
  ...
  async (request) => {
    ...
    logger.info("getFeedbackChartData started", {
      userId,
    ...
    if (totalFeedbacks === 0) {
      logger.info("getFeedbackChartData completed - no data", {
        userId,
    ...
    if (request.data.timeFrame === "range") {
      const dailyAggregates = new Map<
        string,
        { sumU: number; sumI: number; count: number }
      >();
    ...
    logger.info("getFeedbackChartData completed", {
      userId,
      totalFeedbacks,
      durationMs: Date.now() - startTime,
      timestamp: new Date().toISOString(),
...
export const getFeedbackAiSummary = onCall<FeedbackRequestData>(
  ...
  async (request) => {
    ...
    logger.info("getFeedbackAiSummary started", {
      userId,
      employeeId: request.data.employeeId,
      timeFrame: request.data.timeFrame,
      timestamp: new Date().toISOString(),
    ...
    const isAdmin =
      ...
      request.auth.token.isCoAdmin === true;
    if (!isAdmin && request.auth.uid !== request.data.employeeId) {
      logger.warn("Unauthorized AI summary access attempt", {
        userId,
        targetEmployeeId: request.data.employeeId,
      });
      throw new HttpsError(
    ...
    if (comments.length === 0) {
      logger.info("getFeedbackAiSummary completed - no comments", {
        userId,
        durationMs: Date.now() - startTime,
      });
    ...
    try {
      ...
      try {
        ...
        logger.info("getFeedbackAiSummary completed", {
          userId,
...
export const getRawFeedback = onCall<FeedbackRequestData>(
  ...
  async (request) => {
    ...
    logger.info("getRawFeedback started", {
      userId,
      employeeId: request.data.employeeId,
      timeFrame: request.data.timeFrame,
      timestamp: new Date().toISOString(),
    ...
    const isAdmin =
      ...
      request.auth.token.isCoAdmin === true;
    if (!isAdmin && request.auth.uid !== request.data.employeeId) {
      logger.warn("Unauthorized raw feedback access attempt", {
        userId,
        targetEmployeeId: request.data.employeeId,
      });
      throw new HttpsError(
    ...
    logger.info("getRawFeedback completed", {
      userId,
