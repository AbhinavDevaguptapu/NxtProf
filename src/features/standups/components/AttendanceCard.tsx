...
const statusConfig: Record<AttendanceStatus, { label: string; icon: React.ElementType; className: string }> = {
  "Present": { label: "Present", icon: Check, className: "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-200 border-green-200 dark:border-green-800" },
  "Absent": { label: "Absent", icon: UserMinus, className: "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-200 border-red-200 dark:border-red-800" },
  "Missed": { label: "Missed", icon: UserX, className: "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-200 border-amber-200 dark:border-amber-800" },
  "Not Available": { label: "N/A", icon: X, className: "bg-muted text-muted-foreground border-border" },
};
...
const InteractiveControls = ({ employeeId, currentStatus, onSetStatus, onMarkUnavailable }: {
  employeeId: string; // Use a stable ID
  currentStatus: AttendanceStatus;
  onSetStatus: (status: AttendanceStatus) => void;
  onMarkUnavailable: () => void;
}) => {
  const controls: AttendanceStatus[] = ["Present", "Absent", "Missed", "Not Available"];
  return (
            ...
            <TooltipProvider key={status} delayDuration={300}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <motion.button
                    onClick={() => status === "Not Available" ? onMarkUnavailable() : onSetStatus(status)}
                    ...
                    className={cn(
                      ...
                      !isActive && "hover:bg-background/50 text-muted-foreground"
                    )}
                  >
                    {isActive && (
                      <motion.div
                        layoutId="active-pill"
                        className={cn("absolute inset-0 rounded-md border", className)}
                    ...
                    >
                      ...
                      <span className="sm:hidden">{status === "Not Available" ? "N/A" : label}</span>
                    </motion.span>
                  </motion.button>
                </TooltipTrigger>
