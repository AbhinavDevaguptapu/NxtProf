...
const AdminLearningHours = () => {
    ...
    try {
      ...
      if (message === "This session is already synced.") {
        toast({
          ...
          description: `Learning hours for ${format(
            date,
            "PPP"
          )} are already synced.`,
        });
        ...
        message ===
        "No locked learning points found for this session. Marked as synced."
      ) {
        toast({
          title: "No Points to Sync",
          description: `No learning points found for ${format(
            date,
            "PPP"
          )}. Marked as synced.`,
        });
        ...
        toast({
          ...
          description:
            message || `Learning hours processed for ${format(date, "PPP")}.`,
        });
      }
      ...
      toast({
        title: "Sync Failed",
        description:
          "Failed to sync learning hours. Please check the console for more details.",
        variant: "destructive",
      });
    } finally {
      setIsSyncing(false);
  ...
  const filteredLearningPoints = useMemo(() => {
    ...
    return learningPoints.filter(
      (point) => point.userId === selectedEmployeeId
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {format(date, "PPP") === format(new Date(), "PPP")
            ? "Today's"
            : format(date, "PPPP")}{" "}
          Learning Points
          {isAdminUser && isDateSynced && (
        ...
        <CardDescription>
          ...
          {isAdminUser &&
            ` Sync status: ${
              isDateSynced ? "Already synced to spreadsheet" : "Not synced yet"
            }.`}
                ...
                filteredLearningPoints.map((point, index) => (
                  <React.Fragment key={point.id}>
                    <Collapsible
                      asChild
                      key={point.id}
                      open={openRowId === point.id}
                      onOpenChange={() =>
                        setOpenRowId((prevId) =>
                          prevId === point.id ? null : point.id
                        )
                      }
                    >
                      <>
                        <CollapsibleTrigger asChild>
                          <TableRow
                            className={`data-[state=open]:bg-muted/50 hover:bg-muted/50 cursor-pointer ${
                              index % 2 === 0 ? "bg-muted/25" : ""
                            }`}
                          >
                            <TableCell className="font-medium">
                              {employeeMap[point.userId] || "Unknown"}
                            </TableCell>
