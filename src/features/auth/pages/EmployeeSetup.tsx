...
export default function EmployeeSetup() {
  ...
  const isFormValid =
    isEmployeeIdValid(employeeId) && isSheetLinkValid(cleanedSheetLink);
    ...
    try {
      ...
      if (cleanedSheetLink) {
        const urlQuery = query(
          employeesRef,
          where("feedbackSheetUrl", "==", cleanedSheetLink),
        );
        const urlQuerySnapshot = await getDocs(urlQuery);
        if (!urlQuerySnapshot.empty) {
          setError(
            "This Feedback Sheet URL is already associated with another account.",
          );
          setLoading(false);
          return;
        }
      ...
      <motion.div variants={cardVariants} initial="hidden" animate="visible">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              Please provide your Employee ID to continue. The Feedback Sheet
              link is optional.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            >
              ...
              <motion.div variants={fieldVariants} className="space-y-2">
                <label htmlFor="sheetLink" className="text-sm font-medium">
                  Feedback Sheet Link (Optional)
                </label>
                <Input
                  id="sheetLink"
                  type="url"
